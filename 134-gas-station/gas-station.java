class Solution {
    ...
    public boolean check(int[] gas, int[] cost, int currSta, int remaining, int startSta, int travel){
        if(travel>=gas.length) return true;
        if(remaining<=0 && currSta != startSta) return false;
        else if(remaining>=0 && travel>0 && currSta == startSta) return true;
        ...
        if(cal < gas[nextSta]) return false;
